...
async def test_resilience_manager_strict_mode_fail_fast():
    ...
    for b in breakers.values(): b.is_available.return_value = True
    ...
    # Given: Strict routing request (gemini/pro)
    request = ChatRequest(model="gemini/pro", messages=[])
    decision = RoutingDecision(provider=ProviderType.GEMINI, model_name="pro", reason="test")
