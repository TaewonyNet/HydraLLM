...
5. **Web Enrichment** — `services/web_context_service.py` + `services/scraper.py::WebScraper` (Playwright + Scrapling) fetch explicit URLs or perform scraping when web intent is detected, with a 24-hour SQLite cache. When the gateway successfully injects a web context block into `request.messages[-2]`, it emits a stdout INFO line `Web context injected: N chars into request.messages[-2] (session=...)` so operators can confirm enrichment without reading the SQLite event store.
